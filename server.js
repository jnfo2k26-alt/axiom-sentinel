...
const server = http.createServer((req, res) => {
  if (req.url === '/api/reset') {
    // Reset complet — repart à zéro avec les nouveaux paramètres
    const fresh = { ...DEFAULTS, botCreatedAt: Date.now(), portfolioHistory: [2.0], timeLabels: [new Date().toLocaleTimeString('fr-FR')] };
    Object.assign(S, fresh);
    S.positions = [];
    S.history = [];
    S.learn = null;
    S.xp = 0; S.level = 1; S.totalTrades = 0; S.wins = 0; S.losses = 0;
    S.bestTrade = 0; S.worstTrade = 0;
    S.achievements = [];
    S.learningLogs = [{ icon: '🔄', text: 'Reset complet — nouveaux paramètres actifs', time: Date.now() }];
    tick = 0;
    ensureLearningState();
    saveState();
    log('🔄', 'Reset complet effectué');
    res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
    res.end(JSON.stringify({ success: true, message: 'Bot remis à zéro. Portfolio: 2 SOL, SL: 5%, nouveaux paramètres actifs.' }));
